...
describe('Fetch Training Feedback  Reply (E2E)', () => {
  ...
  test('[GET] /trainings/replies', async () => {
    ...
    await prisma.trainingFeedbackReply.create({
      data: {
    ...
    const response = await request(app.getHttpServer())
      .get('/trainings/replies')
      .set('Authorization', `Bearer ${jwtToken}`)
      .send()

    expect(response.status).toBe(200)
    expect(response.body.feedbacksReply).toHaveLength(1)
  })
})
