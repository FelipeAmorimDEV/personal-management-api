...
type FetchStudentExercisesQuerySchema = z.infer<typeof fetchStudentExercisesQuerySchema>

const zodValidationPipe = new ZodValidationPipe(fetchStudentExercisesQuerySchema)
...
export class FetchStudentExercisesController {
  ...
  @Get()
  async handle(@Query(zodValidationPipe) query: FetchStudentExercisesQuerySchema) {
    ...
    const exercises = result.value.exercises.map(HttpStudentExercisePresenter.toHTTP)
