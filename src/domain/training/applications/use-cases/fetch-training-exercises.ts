...
type FetchTrainingExercisesResponse = Either<null, { exercises: StudentExercise[] }>

@Injectable()
export class FetchTrainingExercisesUseCase {
  constructor(private studentExercise: StudentExercisesRepository) {}
  async execute({ trainingId }: FetchTrainingExercisesRequest): Promise<FetchTrainingExercisesResponse> {
   const exercises = await this.studentExercise.fetchManyByTrainingId(trainingId)
